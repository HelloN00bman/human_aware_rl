...
from baselines.ppo3.ppo3 import learn, learn_human_model
...
def two_head_conv_network_fn(**kwargs):
    ...
    def network_fn(X):
        ...
        for _ in range(int(num_hidden_layers / 2)):
            concatenated = tf.layers.Concatenate(axis=1)([out, human_pred_out])
...
@register("two_head_conv_and_mlp2")
def two_head_conv_network_fn(**kwargs):
    """Used to register custom network type used by Baselines for Overcooked"""
    ...
    def network_fn(X):
        ...
        for i in range(1):
            ...
            conv_out = tf.layers.conv2d(
                ...
                padding=padding,
                activation=tf.nn.leaky_relu,
                name="robot_conv_{}".format(i)
            )

        out = tf.layers.flatten(conv_out, name="robot_flatten")

        # for i in range(num_hidden_layers):
        for i in range(2):
            out = tf.layers.dense(out, size_hidden_layers, activation=tf.nn.leaky_relu, name="robot_{}".format(i))
        ...
        for i in range(1):
            ...
            human_pred_out = tf.layers.conv2d(
                ...
                padding=padding,
                activation=tf.nn.leaky_relu,
                name="human_conv_{}".format(i)
            )
        human_pred_out = tf.layers.flatten(human_pred_out, name="human_flatten")
        for i in range(1):
            human_pred_out = tf.layers.dense(human_pred_out, 64, activation=tf.nn.leaky_relu, name="human_dense_{}".format(i))

        human_pred_action = tf.layers.dense(human_pred_out, 6, activation=tf.nn.leaky_relu, name="human_dense_final_6")
        # human_pred_action = tf.layers.dense(human_pred_out, 1, activation=tf.nn.leaky_relu, name="human_dense_final_1")

        print("Last layer conv network output shape", out.shape)  # conv network output shape (30, 64)
        for i in range(int(num_hidden_layers / 2)):
            concatenated = tf.concat([out, human_pred_out, human_pred_action], 1, name="concatenate_both")
            # out = tf.layers.dense(human_pred_out, size_hidden_layers, activation=tf.nn.leaky_relu)
            out = tf.layers.dense(concatenated, size_hidden_layers, activation=tf.nn.leaky_relu, name="merged_{}".format(i))

        out = tf.layers.dense(out, 6, activation=tf.nn.leaky_relu, name="final_robot_action")
        out = tf.concat([out, human_pred_action], 1)
        print("FINAL OUTPUT concat network output shape", out.shape)  #
...
def update_model_for_human(env, model, **kwargs):
    """
    ...
    updated_model, run_info = learn_human_model(
        network=kwargs["NETWORK_TYPE"],
        env=env,
